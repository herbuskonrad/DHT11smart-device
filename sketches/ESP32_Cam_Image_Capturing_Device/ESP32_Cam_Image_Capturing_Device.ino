...
SoftwareSerial mySerialESP(3, 1) //Values correspond to GPIO3(RX) and GPIO1(TX) respectively 
...
#define emailSenderAccount    "senderexample@email.com"    //To use send Email for Gmail to port 465 (SSL), less secure app option should be enabled. https://myaccount.google.com/lesssecureapps?pli=1
#define emailSenderPassword   "senderexamplepassword"

#define emailRecipient        "recipientexample@email.com"
...
void setup() 
{
  ...
  mySerialESP.begin(9600); //data rate for the SoftwareSerial port
  ...
  //Set the subject
  smtpData.setSubject("ESP32-CAM");
    
  //Set the message - normal text or html format
  smtpData.setMessage("Image of greenhouse captured and attached.", true);
...
void loop() 
{
   ...
   while (mySerialESP.available() > 0 && rend == false) {
        rc = mySerialESP.read();
   ...
   strtokIndex = strtok(tempChars,","); // get first part
   t = atoi(strtokIndex); // convert to integer
 
   strtokIndex = strtok(NULL, ","); //continues where the previous call left off
   h = atoi(strtokIndex); // convert to integer

   if (t <= 5 || t > 35){
      String alertMessage = "Warning: Greenhouse temperature has reached critical level, Current Temperature - " + t + "C";

      if (h <= 50 || h > 90){
         String alertMessage = "\nWarning: Greenhouse humidity has reached critical level, Current Humidity - " + h + "%";
      }
      
      emailAlert(alertMessage);
   }
   else if (h <= 50 || h > 90){
      String alertMessage = "Warning: Greenhouse humidity has reached critical level, Current Humidity - " + h + "%";
...
void emailAlert(String alert) {
  ...
  //Set the sender name and Email
  smtpData.setSender("ESP32-CAM Alert", emailSenderAccount);
  
  //Set Email priority or importance High, Normal, Low or 1 to 5 (1 is highest)
  smtpData.setPriority("Normal");

  //Set the subject
  smtpData.setSubject("ESP32-CAM Alert Notification");
    
  //Set the message - normal text or html format
  smtpData.setMessage(alert, true);
